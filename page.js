...
if (typeof document !== "undefined" && !document.getElementById("wh-font")) {
  ...
  l.id = "wh-font"; l.rel = "stylesheet"; l.href = FONT_URL;
...
export default function WHMockup() {
  const [tn, setTn] = useState("Harvest Warm");
  const [expanded, setExpanded] = useState(null);
  const [giveAmt, setGiveAmt] = useState(50);
    ...
    <div style={{ fontFamily: SANS, background: T.bg, color: T.text, minHeight: "100vh" }}>
      ...
      <div style={{
        ...
        {Object.keys(THEMES).map(name => (
          <button key={name} onClick={() => setTn(name)} style={{
      ...
      <section style={{ padding: "80px 28px", background: T.bgAlt }}>
        <div style={{ maxWidth: 540, margin: "0 auto" }}>
          ...
          <input
            ...
            value={customAmt}
            onChange={e => { setCustomAmt(e.target.value); setGiveAmt(null); }}
